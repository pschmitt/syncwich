 ...
 * [updateLabel] must round-trip [LabelEntity.groupId] into the request body (see [LabelUpdateDto]'s
 * kdoc), unlike Categories/Tags/Tools where the id alone is enough.
 */
@Singleton
    ...
    suspend fun updateLabel(labelId: String, name: String, color: String): Result<LabelEntity> =
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                .onFailure {
                    Timber.w(it, "Label update failed for '$labelId'; keeping cached data")
                }
        }
