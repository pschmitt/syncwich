...
constructor(private val organizersApi: OrganizersApi, private val toolDao: ToolDao) {
    ...
    suspend fun createTool(name: String): Result<ToolEntity> = mutate {
        organizersApi.createTool(OrganizerMutationDto(name))
    }

    suspend fun updateTool(toolId: String, name: String): Result<ToolEntity> = mutate {
        organizersApi.updateTool(toolId, OrganizerMutationDto(name))
    }
    ...
    suspend fun deleteTool(toolId: String): Result<Unit> =
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                .onFailure {
                    Timber.w(it, "Tool deletion failed for '$toolId'; keeping cached data")
                }
        }
