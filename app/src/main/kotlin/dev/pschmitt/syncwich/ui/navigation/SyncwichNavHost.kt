...
private val topLevelNavItems =
    listOf(
        ...
        TopLevelNavItem(TopLevelDestination.COOKBOOKS, Icons.AutoMirrored.Filled.MenuBook, "Cookbooks"),
    )
 ...
 */
@Composable
fun SyncwichNavHost(modifier: Modifier = Modifier) {
    ...
    Scaffold(
            ...
            NavigationBar {
                topLevelNavItems.forEach { item ->
                    val selected =
                        currentDestination
                            ?.hierarchy
                            ?.any { it.hasRoute(item.destination.route::class) } == true
                    NavigationBarItem(
                        selected = selected,
                        onClick = {
        ...
        NavHost(
            ...
            startDestination = Route.Recipes,
            ...
            composable<Route.RecipeDetail> {
                // TODO(SW-3): real recipe detail screen, keyed by toRoute<Route.RecipeDetail>().recipeId.
                RecipesScreen()
            }
        }
    }
